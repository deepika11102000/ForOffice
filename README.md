//amount fields with the respective currency field ID's
            const amountFields = [
				{ ccy: 'USANCE_AFTER', amt: 'TENOR_FROM'}
			]; 
		
			const cntyCodeFields = [
			{ cntycd: 'CNTY_CD',  cntynm: 'CNTY_NM'}
			];
		// Stepper data
	const sections = [
		{ id: 'lcFreq', number: 1, title: 'Frequently Amended', tickId: 'tick', numberId: 'number', divid: '0_div' },
		{ id: 'LCDetails', number: 2, title: 'LC Details', tickId: 'tick1', numberId: 'number1', divid: '1_div' },
		{ id: 'BankOtherDetails', number: 3, title: 'Bank & Other Details', tickId: 'tick2', numberId: 'number3', divid: '2_div' },
		{ id: 'ShipmentDetails', number: 4, title: 'Shipment Details', tickId: 'tick3', numberId: 'number4', divid: '3_div' },
		{ id: 'DocumentsRequired', number: 5, title: 'Documents Required & Additional Conditions', tickId: 'tick4', numberId: 'number5', divid: '4_div' },
		{ id: 'Undertakings', number: 6, title: 'Undertakings', tickId: 'tick5', numberId: 'number6', divid: '5_div' },
		{ id: 'DocumentsUpload', number: 7, title: 'Documents Upload', tickId: 'tick6', numberId: 'number7', divid: '6_div' }
...
$(document).ready(function () {
    ...
    $('div[id$="_div"]').each(function () {
			...
			// Check if the input field is currency field to add amount in words
					 if (amountFields.map(field => field.ccy).includes($input.attr('name'))) {
                        // Event listener to update amount in words when the input changes
                        $input.on('change', function () {
						var ccyFieldId = $input.attr('id');

						// Find the corresponding currency field for the current amount field
						var fieldMapping = amountFields.find(field => field.ccy === ccyFieldId);
						
						if (fieldMapping) {
							var amtFieldId = fieldMapping.amt;
							var amountInWordsDiv = document.getElementById(amtFieldId + '_amountInWords');
							var ccyDiv = $('#' + ccyFieldId).parent();

							// Apply high-priority styles
							ccyDiv.attr('style', 'display: flex !important; align-items: top !important; gap: 2% !important; width: 100% !important; height: 60% !important;');
							$('#' + ccyFieldId).attr('style', 'flex: 0 0 24% !important; max-width: 24% !important; min-width: 24% !important; height: 70% !important;');
							$('#' + amtFieldId).attr('style', 'flex: 0 0 74% !important; max-width: 74% !important; min-width: 74% !important; height: 70% !important;');
							ccyDiv.css({ flexWrap: 'wrap' });

							// Update the amount in words
							/* if ($('#' + amtFieldId).val().trim() === "") {
								amountInWordsDiv.textContent = "";
								reArrangeAmtFields();
							} else {
								var amountInWords = amountInWordsNew(ccyFieldId, amtFieldId); // Call the conversion function
								amountInWordsDiv.textContent = amountInWords; // Update the text in the amountInWordsDiv
							} */
						}
					}); 
                    }
					 // Check if the input field is amount field to add amount in words
					else if (amountFields.map(field => field.amt).includes($input.attr('name'))) {
                        // Event listener to update amount in words when the input changes
                        $input.on('input', function () {
						var amtFieldId = $input.attr('id');

						// Find the corresponding currency field for the current amount field
						var fieldMapping = amountFields.find(field => field.amt === amtFieldId);
						
						if (fieldMapping) {
							var ccyFieldId = fieldMapping.ccy;
							var amountInWordsDiv = document.getElementById(amtFieldId + '_amountInWords');
							var ccyDiv = $('#' + ccyFieldId).parent();
                     ...
                     if($input.attr('type') === 'radio'){
						...
						if ($input.hasClass('DATE_M') || $input.hasClass('DATE_P') || $input.hasClass('DATE_O')) {
                            ...
                            var maxLength = $input.attr('maxlength') || cols * rows; // Calculate the maxlength
        ...
        function removeEmptyRowsAndCells() {
            ...
            targetDiv.find('td').each(function () {
                ...
                //var amtFld = $td.hasClass('amt-fld');
				var amtFld = amountFields.find(section => section.amt === fldId);
				var cntynmFld = cntyCodeFields.find(section => section.cntynm === fldId);
                if (isEmpty && !amtFld && !cntynmFld /* && !tolfld */) {
		...
		//Function to format amount fields 
			...
			$.each(amountFields, function(_, sec) {
				...
				// Apply flex styling with high priority for height
				ccyDiv.attr('style', 'display: flex ; align-items: center ; gap: 2% ; width: 100% ;');
				ccy.attr('style', 'flex: 0 0 24% ; max-width: 24% ; min-width: 24% ;');
				amt.attr('style', 'flex: 0 0 74% ; max-width: 74% ; min-width: 74% ;');
				ccyDiv.css({ flexWrap: 'wrap' });
				// Move amount field inside currency div if not already inside
				if (!amt.parent().is(ccyDiv)) {
					ccyDiv.append(amt);
				}
				//add amountinwords span
			/* 	if(ccyDiv.parent().find('#'+sec.amt+'_amountInWords').length === 0){
				var amountInWordsDiv = $('<span>', {
                            id: sec.amt+'_amountInWords',
                            class: 'amount-in-words'
                        });
				
				amountInWordsDiv.innerText= ''
				ccyDiv.parent().append(amountInWordsDiv);
				}
				
				amt.on('blur', function () {
					amountFormat(amt.attr('id'));
				});
		...
		//Function to format Country Code fields 
			function reArrangeCntyCodeFields() {
			$.each(cntyCodeFields, function(_, sec) {
				var cntycd = $('#' + sec.cntycd);
				var cntynm = $('#' + sec.cntynm);
				var cntycdDiv = cntycd.parent();
				// Apply flex styling with high priority for height
				cntycdDiv.attr('style', 'display: flex ; align-items: center ; gap: 2% ; width: 100% ;');
				cntycd.attr('style', 'flex: 0 0 26% ; max-width: 26% ; min-width: 26% ;');
				cntynm.attr('style', 'flex: 0 0 72% ; max-width: 72% ; min-width: 72% ;');
				cntycdDiv.css({ flexWrap: 'wrap' });
				// Move amount field inside currency div if not already inside
				if (!cntynm.parent().is(cntycdDiv)) {
					cntycdDiv.append(cntynm);
				}
...
wrapErrorRows();
	...
	function initializeStepper() {
       ...
       //const useExistingTemplate = $('<span>', { class: 'use-existing-template', text: 'Use Existing Template' ,onclick:'template_IssueLc()' });
        ...
        buttonsContainer.append(cueCardButton, stepByStepVideo);
    ...
    function checkMandatoryFieldsForSections() {
        sections.forEach(section => {
            ...
            div.find('.DATE_M, .CHAR_M, .INT_M').each(function () {
                if ($(this).val() === "") {
			 ...
			 div.find('.AMT_M').each(function () {
                if ($(this).val() === 0 || $(this).val() === "") {
                    allFieldsFilled = false;
                    $(`#${section.id}_warning`).show(); // Show warning if any amt field is empty
                } else {
                    $(`#${section.id}_warning`).hide(); // Hide warning if all amt fields are filled
                    div.find('.number-circle').hide();
                }
            });
    ...
    function renderTrxSysBtnsDiv1() {
        ...
        var $cancelButton = $('<input>', {
            ...
            click: function () {
               // CancelTransaction();
			   getFuncIdOnClick_n('F05030701569')
            }
        }).css({
			'margin-bottom':'auto'
		});
